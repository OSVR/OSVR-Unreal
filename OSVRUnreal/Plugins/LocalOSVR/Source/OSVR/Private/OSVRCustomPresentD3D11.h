...
protected:
	...
	virtual bool LazyOpenDisplayImpl() override
	{
		...
		{
			UE_LOG(FOSVRCustomPresentLog, Warning,
				TEXT("FDirect3D11CustomPresent::LazyOpenDisplayImpl: osvrRenderManagerOpenDisplayD3D11 call failed, or the result status was OSVR_OPEN_STATUS_FAILURE. Potential causes could be that the display is already open in direct mode with another app, or the display does not support direct mode"));
			return false;
		}
		
		UE_LOG(FOSVRCustomPresentLog, Warning,
			TEXT("FDirect3D11CustomPresent::LazyOpenDisplayImpl: Successfully opened the RenderManager display."));
